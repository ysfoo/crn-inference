...
fig = Figure(size = (1000, 1250), figure_padding=1)
a = Axis(fig[1,1])
...
ncolors = Vector{Any}(fill(:black, length(nodes)))
ncolors[[6,17,28]] .= palette[1]
...
xlims!(a, a.finallimits.val.origin[1]-0.0, a.finallimits.val.origin[1]+a.finallimits.val.widths[1]+0.0)
ylims!(a, a.finallimits.val.origin[2]-0., a.finallimits.val.origin[2]+a.finallimits.val.widths[2]-0.2)
labels = [
    begin 
        s = string(rx_vec[rx])
...
Legend(
    fig[1,1],
    fill(MarkerElement(marker=:circle, markersize=0), n_labels+4),
    [labels[1:6]; ""; ""; labels[7:12]; ""; ""; labels[13:20]], "Candidate reactions",
    markerpoints=fill(Point2f(0., 0.), n_labels),
    nbanks=8, orientation=:horizontal,
    padding=(6.,12.,6.,6.),
    gridshalign=:left, labelsize=18, titlesize=18,
