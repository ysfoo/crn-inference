...
for pen_str in PEN_STRS
    ...
    n_base = min(n_crns, 1000)
    base_crns = first.(sort_by_logp[1:n_base])
    ...
    for crn1 in base_crns, crn2 in base_crns
    ...
    display((pen_str, sum(length, values(all_diffs)), n_top, length(cand_logps), n_refine))
    flush(stdout)
